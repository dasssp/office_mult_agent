...
def test_upload_csv_and_reject_formula_injection() -> None:
    ...
    unsafe = client.post(
        "/files/upload", files={"file": ("bad.csv", "name,value\na,=1+1\n", "text/csv")}
    )
...
def test_file_metadata_contains_integrity_hash_and_delete_requires_permission() -> None:
    ...
    deleted = client.delete(f"/files/{file_id}", headers={"x-permission-scopes": "file:delete"})
...
def test_analyze_uploaded_file() -> None:
    ...
    uploaded = client.post(
        "/files/upload",
        files={"file": ("data.json", '[{"amount": 1}, {"amount": null}]', "application/json")},
    )
...
def test_uploaded_file_is_isolated_by_tenant() -> None:
    ...
    uploaded = client.post(
        "/files/upload",
        files={"file": ("private.json", '[{"amount": 1}]', "application/json")},
        headers={"x-tenant-id": "tenant-a"},
    )
    file_id = uploaded.json()["file_id"]
    other_tenant = {"x-tenant-id": "tenant-b"}
    assert client.get(f"/files/{file_id}/metadata", headers=other_tenant).status_code == 404
    assert client.post(f"/analysis/files/{file_id}", headers=other_tenant).status_code == 404
    assert client.post(f"/analysis/files/{file_id}/export", headers=other_tenant).status_code == 404
